...
| -------- | ------- | ----------- |
| `plane_create(normal = [1,0,0], constant = 0)` | `Array` | Creates a new plane. |
...
| `plane_equals(p, p2)` | `boolean` | Returns true if normals and constants match. |
...
| `plane_project_point(p, point, [out])` | `Array` | Projects point onto plane. |
| `plane_distance_to_point(p, point)` | `number` | Signed distance from point to plane. |
