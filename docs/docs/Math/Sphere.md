...
| -------- | ------- | ----------- |
| `sphere_create(center, r)` | `Array` | Creates a new sphere (defaults to `[0,0,0,-1]`). |
...
| `sphere_contains_point(s, point)` | `boolean` | True if point is inside or on the sphere. |
| `sphere_intersects_sphere(a, b)` | `boolean` | True if spheres intersect. |
| `sphere_distance_to_point(s, point)` | `number` | Distance from surface to point (negative if inside). |
| `sphere_clamp_point(s, point, out?)` | `Array` | Clamps a point to sphere boundary; returns `out` or new vec. |
