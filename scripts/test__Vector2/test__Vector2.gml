...
suite(function () {
    describe("Vec2", function () {
        ...
        test("vec2_distance_to_segment() computes distance correctly", function () {
            var p, a, b, dist;

            // Point above the horizontal segment 
            p = [1, 2]; a = [0, 0]; b = [2, 0];
            dist = vec2_distance_to_segment(p, a, b);
            expect(dist).toBe(2);

            // Point outside the segment, near a 
            p = [-1, 0]; a = [0, 0]; b = [2, 0];
            dist = vec2_distance_to_segment(p, a, b);
            expect(dist).toBe(1);

            // Point outside the segment, near b 
            p = [3, 0]; a = [0, 0]; b = [2, 0];
            dist = vec2_distance_to_segment(p, a, b);
            expect(dist).toBe(1);
