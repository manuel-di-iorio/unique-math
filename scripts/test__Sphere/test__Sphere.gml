...
suite(function() {
    describe("Sphere", function() {
        test("sphere_create()", function() {
            var s = sphere_create(vec3_create(1, 2, 3), 4);
            expect(s[0]).toBe(1);
            expect(s[1]).toBe(2);
            expect(s[2]).toBe(3);
            expect(s[3]).toBe(4);
        ...
        test("sphere_set() and sphere_copy()", function() {
            var s = sphere_create();
            sphere_set(s, vec3_create(1, 2, 3), 4);
            expect(s[0]).toBe(1);
        ...
        test("sphere_contains_point()", function() {
            var s = sphere_create(vec3_create(0, 0, 0), 10);
            expect(sphere_contains_point(s, vec3_create(0, 0, 0))).toBeTruthy();
            expect(sphere_contains_point(s, vec3_create(10, 0, 0))).toBeTruthy();
            expect(sphere_contains_point(s, vec3_create(11, 0, 0))).toBeFalsy();
        });
        
        test("sphere_distance_to_point()", function() {
            var s = sphere_create(vec3_create(0, 0, 0), 10);
            expect(sphere_distance_to_point(s, vec3_create(5, 0, 0))).toBe(-5);
            expect(abs(sphere_distance_to_point(s, vec3_create(15, 0, 0)) - 5) < 0.001).toBeTruthy();
        });
        
        test("sphere_clamp_point() outside clamps to surface", function() {
            var s = sphere_create(vec3_create(0, 0, 0), 10);
            var out = sphere_clamp_point(s, vec3_create(20, 0, 0));
        ...
        test("sphere_clamp_point() inside unchanged", function() {
            var s = sphere_create(vec3_create(0, 0, 0), 10);
            var out = sphere_clamp_point(s, vec3_create(5, 0, 0));
            expect(abs(out[0] - 5) < 0.001).toBeTruthy();
        });
        
        test("sphere_expand_by_point()", function() {
            var s = sphere_create(vec3_create(0, 0, 0), 5);
            sphere_expand_by_point(s, vec3_create(10, 0, 0));
            expect(abs(s[3] - 10) < 0.001).toBeTruthy();
        });
        
        test("sphere_is_empty() and make_empty()", function() {
            var s = sphere_create(vec3_create(0, 0, 0), -1);
        ...
        test("sphere_translate()", function() {
            var s = sphere_create(vec3_create(0, 0, 0), 5);
            sphere_translate(s, vec3_create(10, 10, 10));
            expect(s[0]).toBe(10);
            expect(s[1]).toBe(10);
            expect(s[2]).toBe(10);
        });
        ...
        test("sphere_get_bounding_box() contains extremes", function() {
            ...
            expect(box3_contains_point(b, vec3_create(10, 0, 0))).toBeTruthy();
            expect(box3_contains_point(b, vec3_create(-10, 0, 0))).toBeTruthy();
            expect(box3_contains_point(b, vec3_create(0, 10, 0))).toBeTruthy();
            expect(box3_contains_point(b, vec3_create(0, 0, -10))).toBeTruthy();
        });
        
        test("sphere_intersects_box()", function() {
            var s = sphere_create(vec3_create(0, 0, 0), 5); 
            var b = box3_create(vec3_create(4, -1, -1), vec3_create(6, 1, 1));
            expect(sphere_intersects_box(s, b)).toBeTruthy();
            var b2 = box3_create(vec3_create(6, -1, -1), vec3_create(8, 1, 1));
        ...
        test("sphere_union()", function() {
            var s1 = sphere_create(vec3_create(0, 0, 0), 5);
            var s2 = sphere_create(vec3_create(10, 0, 0), 5);
            sphere_union(s1, s2);
            expect(sphere_contains_point(s1, vec3_create(0, 0, 0))).toBeTruthy();
            expect(sphere_contains_point(s1, vec3_create(10, 0, 0))).toBeTruthy();
        });
        ...
        test("sphere_set_from_points()", function() {
            ...
            expect(sphere_contains_point(s, vec3_create(0, 0, 0))).toBeTruthy();
            expect(sphere_contains_point(s, vec3_create(10, 0, 0))).toBeTruthy();
            expect(sphere_contains_point(s, vec3_create(0, 10, 0))).toBeTruthy();
        });
        
        test("sphere_intersects_sphere()", function() {
            var s1 = sphere_create(vec3_create(0, 0, 0), 5);
            var s2 = sphere_create(vec3_create(10, 0, 0), 4); // Dist 10, radii sum 9 -> False
            expect(sphere_intersects_sphere(s1, s2)).toBeFalsy();
            
            var s3 = sphere_create(vec3_create(9, 0, 0), 4); // Dist 9, radii sum 9 -> True
